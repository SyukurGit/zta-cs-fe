...
export function ChatInterface({ ticketId, className }: ChatInterfaceProps) {
  ...
  // 1. Fetch History Chat
  const fetchChat = async () => {
    try {
      // Endpoint: GET /api/{role}/tickets/:id/chat
  ...
  // Polling setiap 3 detik (Simulasi Realtime)
  useEffect(() => {
    fetchChat();
      ...
      <CardContent className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50">
        {messages.length === 0 && (
          <div className="text-center text-slate-400 text-sm mt-10">
            Belum ada percakapan. Mulai sapa {role === 'CS' ? 'User' : 'CS'}!
          </div>
        )}
